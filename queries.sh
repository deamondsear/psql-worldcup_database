...
echo  "$($PSQL "SELECT sum_of_goals_of_winners + sum_of_goals_of_opponents 
FROM (SELECT SUM(games.winner_goals) AS sum_of_goals_of_winners, 
SUM(games.opponent_goals) AS sum_of_goals_of_opponents FROM games) AS total")"
